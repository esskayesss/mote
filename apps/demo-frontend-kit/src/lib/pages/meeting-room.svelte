<script lang="ts">
  ...
  import MeetingView from "$lib/pages/meeting-view.svelte";
  ...
  let transcriptionConfig = $state<{
    url: string;
    provider: RoomSummary["transcriptionProvider"];
    sampleRate: number;
    language: string;
    model: string;
  } | null>(null);
  ...
  const currentCode = $derived(meetCode);
  ...
  const applyMeetingEvent = (event: MeetingEvent) => {
    switch (event.type) {
      ...
      case "moderation.participant_removed": {
        ...
        const nextParticipantId = event.payload.participantId;
  ...
  const syncRoom = async (code: string) => {
    ...
    if (
      storedParticipantId &&
      data.room.participants.some((participant) => participant.id === storedParticipantId)
    ) {
      participantId = storedParticipantId;
    } else if (
      participantId &&
      data.room.participants.some((participant) => participant.id === participantId)
    ) {
  ...
  const joinMeeting = async () => {
    ...
    try {
      const data = await roomsApi.joinRoom(currentCode, displayName);
      room = data.room;
  ...
  const acknowledgeFactCheck = async (event: FactCheckPrivateEvent) => {
    ...
    try {
      ...
      errorMessage = error instanceof Error ? error.message : "Unable to acknowledge fact check.";
    } finally {
  ...
  const leaveMeeting = () => {
    if (currentCode && participantId) {
      void roomsApi.leaveRoom(currentCode, participantId);
      localStorage.removeItem(participantStorageKey(currentCode));
    }
  ...
  $effect(() => {
    if (localStream || mediaState === "requesting" || mediaState === "blocked") {
      return;
    }
  ...
  onMount(() => {
    const onBeforeUnload = () => {
      const activeParticipantId = localStorage.getItem(participantStorageKey(currentCode));

      if (!currentCode || !activeParticipantId) {
        return;
      }

      navigator.sendBeacon(
        `${backendUrl}/rooms/${currentCode}/leave`,
...
<MeetingView
  ...
  onToggleMoteMonitorMessages={() => (showMoteMonitorMessages = !showMoteMonitorMessages)}
