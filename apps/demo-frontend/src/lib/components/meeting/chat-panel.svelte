  ...
  {#if chatMessages.length}
    ...
    >
      {#each chatMessages as event}
        <div class="border border-border bg-panel-subtle px-4 py-4 text-sm leading-6 text-foreground-soft">
          ...
          <p class="text-sm leading-6 text-foreground-soft">{event.payload.message}</p>
        </div>
      {/each}
    </div>
  {:else}
